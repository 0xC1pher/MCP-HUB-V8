...
class DynamicChunkingSystem:
    ...
    def chunk_text(self,
                    text: str,
                    file_path: str = "",
                    metadata: Dict[str, Any] = None) -> List[Tuple[int, int, str]]:
        """Compatibilidad v5: expone una API tipo chunk_text.

        Devuelve una lista de tuplas (start_line, end_line, section) basada en
        los metadatos ya calculados por adaptive_chunking, sin modificar la
        lógica de chunking.
        """

        adaptive_chunks = self.adaptive_chunking(text, file_path, metadata)
        ranges: List[Tuple[int, int, str]] = []

        for chunk in adaptive_chunks:
            md = chunk.metadata

            # En DynamicChunkingSystem las líneas son 1-based, en v5 se usan 0-based
            start = max(md.line_start - 1, 0)
            end = max(md.line_end - 1, start)

            # El campo section original de v5 no se usa en la lógica de negocio,
            # así que dejamos cadena vacía para mantener compatibilidad de firma.
